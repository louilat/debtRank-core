...
def extract_data(client_s3, day: datetime) -> tuple[DataFrame, DataFrame]:
    ...
    data = client_s3.get_object(
        Bucket="debtrank",
        Key=f"debtrank-inputs/debtrank_inputs_snapshot_date={day_str}/nodes_info.csv",
    )["Body"]
    nodes_info = pd.read_csv(data)

    data = client_s3.get_object(
        Bucket="debtrank",
        Key=f"debtrank-inputs/debtrank_inputs_snapshot_date={day_str}/nodes_impacts.csv",
    )["Body"]
    ...
    data = client_s3.get_object(
        Bucket="debtrank",
        Key=f"debtrank-inputs/debtrank_inputs_snapshot_date={day_str}/prices_shocks.csv",
    )["Body"]
